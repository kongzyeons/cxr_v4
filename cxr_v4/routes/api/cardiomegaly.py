...
colors = ["red" for i in range(len(list_class))]

print('Start Run Api Cardiomegaly ....')
...
@router.post("/predict_cardiomegaly_base64", status_code = 201)
async def get_base64(file_base64: str = Form(...),confident: float = Form(...)):
    try:
        ...
        if 0<=confident<=1:
            ...
            count_detect = 0
            if len(boxs) != 0 and dict_class['Cardiomegaly'] in labels:
                for i in range(len(batch_detections[0])):
                    if int(batch_detections[0][i][5]) == dict_class['Cardiomegaly']:
                ...
                return {
                    ...
                    "output_detail" :  {
                        ...
                        "Cardiomegaly":f"{100*pred_cardio[0][0][4]:.2f}%",
                        "img_base64":img_base64['img_base64']
                    }
                    }
...
@router.post("/predict_cardiomegaly_image", status_code = 201)
